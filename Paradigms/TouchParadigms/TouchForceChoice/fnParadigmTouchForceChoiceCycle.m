...
% 
% Plays animation during choices period
%
...
global g_strctParadigm  g_strctDAQParams g_strctPTB
...
switch g_strctParadigm.m_iMachineState
    ...
    case 4
        g_strctParadigm.m_iMachineState = 10; % JCL
%         % Show fixation spot
    ...
    case 15
        ...
        if bGiveJuice
            ...
            if g_strctParadigm.m_strctCurrentTrial.m_strctChoices.m_bMultipleAttemptsUntilJuice
                ...
                % Incorrect trial
                % Air puff 
                fnDAQ('TTL',g_strctDAQParams.m_fPuffPort,1);
                % JCL
        ...
        if bAbortTrial
            ...
            if ~isempty(strctInputs.m_acInputFromStimulusServer)
                if strcmpi(strctInputs.m_acInputFromStimulusServer{1},'ChoicesOnsetTS')
                    ...
                    %Added by XQ
                    g_strctParadigm.m_strSelectedDirection = '';
                    g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter = 1;                    
                    g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter) = fnDAQWrapper('GetAnalog', g_strctDAQParams.m_fConveyerPort);
                    %Ended by XQ
...
% %Changed by XQ
% 
%          if isempty(g_strctParadigm.m_strSelectedDirection)
%             g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter = g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter+1;                    
%             g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter) = fnDAQWrapper('GetAnalog', g_strctDAQParams.m_fConveyerPort);
%             %fprintf('%f\n', g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter));
%             if abs(g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter)- ...
%                 g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter-1)) > g_strctDAQParams.m_fVoltageDiffThreshold
%                 bInsideChoice = true;
%             
% %!!!!!!!!!!!Check the relationship between the slope and direction !!!!!!!!!!!!!!!            
%                 if g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter)>...
%                    g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter-1)
%                       g_strctParadigm.m_strSelectedDirection = 'right';
%                 else
%                       g_strctParadigm.m_strSelectedDirection = 'left';
%                 end
%             else
%                 bInsideChoice = false;
%             end
...
%        
%            if bInsideChoice
%                 g_strctParadigm.m_fInsideChoiceTS = GetSecs();  %here it means making the right choice
%                 if strcmp(g_strctParadigm.m_strSelectedDirection, 'left')
%                    [dummyMin, g_strctParadigm.m_iSelectedChoice] = min([g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(1).m_pt2fPosition(1) g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(2).m_pt2fPosition(1)]);
%                 else
%                    [dummyMax, g_strctParadigm.m_iSelectedChoice] = max([g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(1).m_pt2fPosition(1) g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(2).m_pt2fPosition(1)]);
%                 end
%                 
...
case 21
        ...
        g_strctParadigm.bMovie = 1;
              
        % CONTINUE PLAYING "MOVIE UNTIL CHOICES PERIOD ENDS
        
        if fElapsedTime < g_strctParadigm.m_strctCurrentTrial.m_strctPostTrial.m_fTrialTimeoutMS/1e3 && ~bInsideChoice           
% CHECKING VOLTAGE            
            if isempty(g_strctParadigm.m_strSelectedDirection)
                        g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter = g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter+1;                    
                        g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter) = fnDAQWrapper('GetAnalog', g_strctDAQParams.m_fConveyerPort);
                        ...
                        if abs(g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter)- ...
                            g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter-1)) > g_strctDAQParams.m_fVoltageDiffThreshold
                            bInsideChoice = true;
                            g_strctParadigm.bMovie = 0; % JCL 10/17/13
...
% END CHOICES PERIOD - EITHER TIMEOUT OR CHOICE SELECTED
% Also clear rect matrix and clear screen

        elseif fElapsedTime > g_strctParadigm.m_strctCurrentTrial.m_strctPostTrial.m_fTrialTimeoutMS/1e3
            g_strctParadigm.bMovie = 0;
            g_strctParadigm.m_iMachineState = 24;
   
        elseif bInsideChoice
                g_strctParadigm.m_fInsideChoiceTS = GetSecs();  %here it means making the right choice
                if strcmp(g_strctParadigm.m_strSelectedDirection, 'left')
                   [dummyMin, g_strctParadigm.m_iSelectedChoice] = min([g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(1).m_pt2fPosition(1) g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia(2).m_pt2fPosition(1)]);
                else
...
% XQ
         g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter = g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter+1;                    
         g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter) = fnDAQWrapper('GetAnalog', g_strctDAQParams.m_fConveyerPort);
         fLastVoltage = g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter-1);
         fCurrentVoltage = g_strctParadigm.m_strctCurrentTrial.m_afAnalogSignal(g_strctParadigm.m_strctCurrentTrial.m_iAnalogSignalCounter);
         if strcmp(g_strctParadigm.m_strSelectedDirection, 'right')
              if fCurrentVoltage >= fLastVoltage || (fLastVoltage-fCurrentVoltage) <= g_strctDAQParams.m_fVoltageDiffThreshold
                 bInsideChoice = true;
              else
                 bInsideChoice = false;
                 g_strctParadigm.m_strSelectedDirection = 'left';        %Flip the choice        
              end
         else
              if fCurrentVoltage <= fLastVoltage || (fCurrentVoltage - fLastVoltage) <= g_strctDAQParams.m_fVoltageDiffThreshold
                 bInsideChoice = true;
              else
                 bInsideChoice = false;
                 g_strctParadigm.m_strSelectedDirection = 'right';                 %Flip the choice
              end
         end
...
% function [ output_args ] = movieplay(hPTBWindow)
