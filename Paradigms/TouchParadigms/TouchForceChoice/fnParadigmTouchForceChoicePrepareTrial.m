    ...
    case 'random'
       ...
       % attributes.
        ...
        if iNumChoicesWithSimilarAttrToCue > 0
            ...
            %Changed by XQ
            aiRand = [];
            while length(aiRand)~=iNumValidChoices
                 aiRand = randi(iNumValidChoices, [1,iNumChoices]);
                 aiRand = unique(aiRand);
            end
            %Ended by XQ
            % Select choices
            if bCueIsChoice
...
if isfield(strctRoot,'StimulationPreset')
    ...
    for iStimIter=1:iNumChannels
        ...
        %astrctMicroStim(iStimIter).m_fAmplitude =  fnParseVariable(strctStimPreset,'Amplitude', 0,iStimIter); 
        astrctMicroStim(iStimIter).m_fAmplitude = g_strctDAQParams.m_afMicroStimAmplitude(astrctMicroStim(iStimIter).m_iChannel);
