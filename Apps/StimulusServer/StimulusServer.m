...
%Play sound if the variable PlaySound in stimulus server config is 1 
if g_strctConfig.m_strctStimulusServer.m_fPlaySound==1
    % Attempt to create audio device using ASIO interface
    InitializePsychSound(1);
    astrctAudioDevices = PsychPortAudio('GetDevices');
    iDeviceIndex = 2; %find(ismember({astrctAudioDevices.DeviceName},g_strctConfig.m_strctStimulusServer.m_strAudioDeviceName));
    ...
    iAgressiveMode = 4;
    try
        Fs1 = g_strctConfig.m_strctStimulusServer.m_fSamplingFreq;
        t=0:1/Fs1:(g_strctConfig.m_strctStimulusServer.m_fSoundDuration-1/Fs1);
        freq = g_strctConfig.m_strctStimulusServer.m_fSoundFreq;%Hz
        f1 = sin(2*pi*freq*t)*0.3;
        Channels = size(f1,1);
        g_strctPTB.m_hAudioDevice = PsychPortAudio('Open', [], [], iAgressiveMode, Fs1, Channels);
        PsychPortAudio('FillBuffer', g_strctPTB.m_hAudioDevice,f1);
        
        g_strctPTB.SoundPlayed = 0;

    catch
        g_strctPTB.m_hAudioDevice = [];
        error('Fail to initialize audio');
    end
end
