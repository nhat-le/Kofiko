...
% Last Modified by GUIDE v2.5 26-Mar-2012 09:14:32
...
strTemplate = fullfile(pwd,'Solidworks','Grid_Template.SLDPRT');
strOutputFile = fullfile(pwd,'Solidworks','Grid_Modified.SLDPRT');

   prompt={'Hole radius (mm):','Wrapped text:','Template file:','Output file:'};
   name='Model options';
   numlines=1;
   defaultanswer={'0.35','Planner Model v1.0',strTemplate,strOutputFile};
    answer=inputdlg(prompt,name,numlines,defaultanswer,'on');
 if isempty(answer)
     return;
 end;

fRadiusMM = str2num(answer{1});
strWrappedText = answer{2};
strTemplate = answer{3};
strOutputFile = answer{4};
