...
%
% Handles Key Events like manual juice (among other things).
%
...
% This is very important signal that will be later used to synchornize
...
%% Listen to what Ardudino has to say
...
if fCycleTic - g_strctCycle.m_fKeyboardMouseInteractionTimer > g_strctCycle.m_fKeyboardMouseInteractionMS/1e3
    ...
    try
        [fMouseX, fMouseY, aiButtons] = GetMouse(g_strctPTB.m_iScreenIndex);  % This takes 0.06 ms
    catch
        [fMouseX, fMouseY] = GetMouse(g_strctPTB.m_iScreenIndex);
        aiButtons = [0,0,0];
    end
...
if g_strctDAQParams.m_fUseMouseClickAsEyePosition || g_strctDAQParams.m_bMouseGazeEmulator
    try
        [fEyeXPix, fEyeYPix, aiButtons] = GetMouse(g_strctDAQParams.m_fMonitorIndexForEyePosition);
    catch
        [fEyeXPix, fEyeYPix] = GetMouse(g_strctDAQParams.m_fMonitorIndexForEyePosition);
        aiButtons = [0,0,0];
...
if ~g_strctCycle.m_bParadigmPaused
    feval(g_strctParadigm.m_strCycle,strctInputs);% This takes 0.1ms for the Passive fixation
end;
...
if g_strctGUIParams.m_bUseSpecialKeys
    if keyIsDown && g_strctCycle.m_iKeyFSM == 0
        if keyCode(g_strctGUIParams.m_fJuiceKey)
            fnParadigmToKofikoComm('newjuice', g_strctGUIParams.m_fJuiceTimeMS, true);
        end;
        % To get juice through tube without counting it towards juice total
        if keyCode(g_strctGUIParams.m_fSetupJuiceKey)
            fnParadigmToKofikoComm('setupjuice');
        end;
...
% Using directly using fnDAQ instead 1/22/2014
...
if (fCurrTime-g_strctCycle.m_fScreenTimer) > g_strctGUIParams.m_fRefreshRateMS / 1000 && ...
    ...
    if ~g_strctCycle.m_bParadigmPaused
        feval(g_strctParadigm.m_strDraw);
    end
   ...
   %afCycleDebugTimers(41) = GetSecs();
    ...
    %% Write current performance on the screen
    
    if isfield(g_strctParadigm,'StimulusIndex')
        num_trials = numel(find(g_strctParadigm.StimulusIndex.Buffer>0))-1;
        num_target = numel(find(g_strctParadigm.StimulusIndex.Buffer==1))-1;
        num_distractors = num_trials - num_target;
        num_incorrect = numel(find(g_strctParadigm.IncorrectStop.Buffer>0));
        num_correct = numel(find(g_strctParadigm.CorrectStop.Buffer>0));
    ...
    Screen(g_strctPTB.m_hWindow,'DrawText',sprintf('No. trials = %d.',num_trials), ...
        g_strctPTB.m_aiRect(1)+100,g_strctPTB.m_aiRect(4)-70, [255 0 255]);
    
    Screen(g_strctPTB.m_hWindow,'DrawText',sprintf('Targets         = %d.',num_target), ...
        g_strctPTB.m_aiRect(1)+400,g_strctPTB.m_aiRect(4)-70, [255 0 255]);
    
    Screen(g_strctPTB.m_hWindow,'DrawText',sprintf('Distractors          = %d',num_distractors), ...
        g_strctPTB.m_aiRect(1)+800,g_strctPTB.m_aiRect(4)-70, [255 0 255]);
    
    
    
    Screen(g_strctPTB.m_hWindow,'DrawText',sprintf('Stops on target = %d (%.0f ml).',num_correct, ...
        g_strctGUIParams.m_fJuiceTimeOpenTotalMS / g_strctGUIParams.m_fJuiceMl_To_Ms),...
        g_strctPTB.m_aiRect(1)+400,g_strctPTB.m_aiRect(4)-50, [255 0 255]);
    
    Screen(g_strctPTB.m_hWindow,'DrawText',sprintf('Stops on distractors = %d',num_incorrect), ...
        g_strctPTB.m_aiRect(1)+800,g_strctPTB.m_aiRect(4)-50, [255 0 255]);
...
%% Measure cycle timeing
fCurrTime = fCycleTic;
if fCurrTime-g_strctCycle.m_fRateTimer > 1 % Update every one sec
    % We have three measures for cycle timeing:
