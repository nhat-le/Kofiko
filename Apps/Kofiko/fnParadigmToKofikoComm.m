...
switch lower(strCommand) 
    case 'newjuice'
       ...
       addpath('C:\Kofiko\MEX\x64');
       fnDAQ('TTL',g_strctDAQParams.m_fJuicePort, fRewardOpenTimeMS/1e3);
       g_strctGUIParams.m_iJuiceCounter = g_strctGUIParams.m_iJuiceCounter + 1;
       g_strctGUIParams.m_fJuiceTimeOpenTotalMS = g_strctGUIParams.m_fJuiceTimeOpenTotalMS + fRewardOpenTimeMS;
       if nargin >= 3
            g_strctCycle.m_strctJuiceProcess.m_bManual = varargin{2} > 0;
       end
    % key press "K" will open valve so that juice will reach spout before experiments start   
    case 'setupjuice'
       addpath('C:\Kofiko\MEX\x64');
       fnDAQ('TTL', g_strctDAQParams.m_fJuicePort, 90/1e3);
           ...
           % Set the triggering Finite state machines
             ...
             for iIter=1:iNumChannelsNeedTrigger
                 if ~isfield(astrctTriggerInfo(iIter),'m_fAmplitude') || ...
                     ...
                     astrctTriggerInfo(iIter).m_fAmplitude = g_strctDAQParams.m_afMicroStimAmplitude(astrctTriggerInfo(iIter).m_iChannel);
                 end
