...
g_handles.m_strctSettingsPanel.m_hMicroStimChAll = uicontrol('Style','pushbutton','String','Microstim Pulse (All)',...
     'Position',[80 iLowerPanelHeight-690 150 30],'HorizontalAlignment','Left','Parent',...
...
g_handles.m_strctSettingsPanel.m_hMicroStimGUI = uicontrol('Style','pushbutton','String','Microstim GUI',...
     'Position',[80 iLowerPanelHeight-720 150 30],'HorizontalAlignment','Left','Parent',...
...
%         end;
    ...
    case 'MicrostimAmplitude'
        ...
        if length(fAmplitude) == 1 && ~isempty(fAmplitude)
            g_strctDAQParams.m_afMicroStimAmplitude(iHandle) = fAmplitude;
        end
    case 'MicroStimPulse'
        aiChannels = varargin{1};
        for k=1:length(aiChannels)
            fnParadigmToKofikoComm('StimulationTTL',aiChannels(k),g_strctDAQParams.m_afMicroStimAmplitude(aiChannels(k)));
        end
    case 'MicroStimGUI'
        %NanoStimulatorGUI();
...
g_strctGUIParams.m_iTrialCounter = 0;
...
g_strctDAQParams.m_afMicroStimAmplitude = [NaN,NaN];
