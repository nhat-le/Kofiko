...
% Last Modified by GUIDE v2.5 08-Sep-2011 14:04:27
...
switch eventdata.Key
    ...
    case 'space'
        ...
        if ~isempty(eventdata.Modifier) && strcmp(eventdata.Modifier{1},'shift')
            ...
            [coeff,ignore] = eig(a2fSortedAllWaveFormsZeroMean(abValidSpikes,:)'*a2fSortedAllWaveFormsZeroMean(abValidSpikes,:));
...
%%
    ...
    aiClusters = fndllKlustaKwikMatlabWrapper(a2fPCAFeatures',[],'MinClusters',1,'MaxClusters',iMaxCluster,'MaxPossibleClusters',iMaxCluster,'Verbose',0);
...
for iIter=1:iNumUnitsInRange
    ...
    if isempty(iIndex) ||  (~isempty(iIndex) && astrctUnitIntervals(iIndex).m_bVisible == true)
        ...
        a2fSmooth = conv2(a2fDist,a2fSmoothingKernel,'same');
        a3fSmoothDist(:,:,iIter) = a2fSmooth / max(a2fSmooth(:));
...
function hNeigh_Callback(hObject, eventdata, handles)
a2fPCA = getappdata(handles.figure1,'a2fPCA');
acUnitAssociation = getappdata(handles.figure1,'acUnitAssociation');
a2fSortedAllWaveForms = getappdata(handles.figure1,'a2fSortedAllWaveForms');
afSortedAllTS = getappdata(handles.figure1,'afSortedAllTS');
