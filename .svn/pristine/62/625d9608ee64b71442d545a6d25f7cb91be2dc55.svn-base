...
fLastFlipTime = Screen('Flip',g_strctPTB.m_hWindow); % This would block the server until the next flip.
...
if hFrameTexture == -1
    ...
    Screen('FillArc',g_strctPTB.m_hWindow,[255 255 255], aiFixationRect,0,360);
    fLastFlipTime = Screen('Flip',g_strctPTB.m_hWindow);  % Block (!)
...
%     else
        ...
        fLastFlipTime = Screen('Flip',g_strctPTB.m_hWindow, g_strctDraw.m_fMovieOnset+fTimeToFlip);  % Block (!)
        Screen('Close', hFrameTexture);
