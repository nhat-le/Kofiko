		...
		void fnHandleTCPIP() {
			...
			if (ClientConnected) {
				if (!Active_Client.connected()) {
					...
					while (Active_Client.available()) {

						char c = Active_Client.read();
						//        Serial.print("Buffer: ");Serial.print(buffer_index);
						//        Serial.print("Recved: ");Serial.println(c);
						packetBuffer[buffer_index++] = c;
						...
						if (c == 0) {
							...
							if (successful)
								Active_Client.println(String("OK! ")+String(packetBuffer));   
							else
								Active_Client.println(String("NOK ")+String(packetBuffer));   
