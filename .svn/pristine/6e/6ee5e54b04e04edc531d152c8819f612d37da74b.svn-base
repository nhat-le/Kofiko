function acUnitsStat = fnCollectPassiveFixationNewUnitStats(strctKofiko, strctSync, strctConfig, strctInterval)
...
for iListIter=1:iNumUniqueLists
    ...
    if ~isempty(aiRelevantTrialsInd)
        strctUnit = fnCollectPassiveFixationNewUnitStatsAux(...
            strctKofiko, strctSync, strctConfig, strctInterval, strListName, aiRelevantTrialsInd, iParadigmIndex);
...
function strctUnit = fnCollectPassiveFixationNewUnitStatsAux(...
    strctKofiko, strctSync, strctConfig, strctInterval, strListName, aiTrialIndices, iParadigmIndex)
