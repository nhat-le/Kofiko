...
for iTrainIter=1:length(astrctUniqueTrains)
    ...
    if sum(astrctUniqueTrains(iTrainIter).m_iPulsesPerTrain == 1) > 0 && ...
        ...
        fMode = mode(astrctUniqueTrains(iTrainIter).m_afPulseLengthMS);
        aiTrainSubset = find( abs(astrctUniqueTrains(iTrainIter).m_afPulseLengthMS -fMode) < 1);
    ...
    abBefore = aiPeriStimulusRangeMS<=0;
    abDuring = aiPeriStimulusRangeMS>0 & aiPeriStimulusRangeMS <= astrctUniqueTrains(iTrainIter).m_fTrainLengthMS;
    abAfter = aiPeriStimulusRangeMS > astrctUniqueTrains(iTrainIter).m_fTrainLengthMS;
