...
if bDropInvalidTrains
    abValidTrains = sum(a2fTrainValues > fThreshold,2) > 0;
else
    abValidTrains = ones(1,iNumTriggers)>0;
end

fnWorkerLog('%d Triggers were detected', iNumTriggers);
fnWorkerLog('%d Trains were detected right after those trigger (%d missed trains!)',sum(abValidTrains), sum(~abValidTrains));
