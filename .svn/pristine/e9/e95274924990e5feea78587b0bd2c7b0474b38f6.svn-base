...
a2fTargetCenter = a2fUniqueCuePositions(aiRemapToCanonical,:);
   ...
   acTrials=fnReanalyzeSaccadeTrials(acTrials,strRawFolder,strSession,strctSync,strctKofiko,a2fTargetCenter,fFixationRadius,fChoiceRadius);
    ...
    for iUniqueTrialIter=1:iNumUniqueTrialTypes
        ...
        if ~isempty(acTrialsOfSameType{k}.m_strctNewTrialOutcome)
            acTrialOutcome{k} = acTrialsOfSameType{k}.m_strctNewTrialOutcome.m_strOutcome;
        else
