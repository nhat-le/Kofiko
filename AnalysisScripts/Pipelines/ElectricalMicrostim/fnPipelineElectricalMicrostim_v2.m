function fnPipelineElectricalMicrostim_v2(strctInputs)
...
abSimulatenousTrains1 = afMinTimeSec < 2*1e-3;
    ...
    abSimulatenousTrains1 = zeros(1,iNumTrainsGrass1)>0;
end
...
abSimulatenousTrains2 = afMinTimeSec < 2*1e-3;
    ...
    abSimulatenousTrains2 = zeros(1,iNumTrainsGrass2)>0;
end
...
for iDepthIter=1:length(afUniqueDepthMM)
    ...
    abRelevantFixationOnScreen = abFixationOnScreen(aiTrainIndices);
    ...
    abFixationGrass1 = abRelevantFixationOnScreen(abTrainsCh1 &~abAtSameTime);
    afTrainsGrass2TS = afTrainsTS(abTrainsCh2 &~abAtSameTime);
    abFixationGrass2 = abRelevantFixationOnScreen(abTrainsCh2 &~abAtSameTime);
    afTrainsBothGrass = afTrainsTS(abTrainsCh1&abAtSameTime);
    abFixationGrassBoth = abRelevantFixationOnScreen(abTrainsCh1&abAtSameTime);
...
%     end
    ...
    if length(afTrainsGrass1TS) > 0
        fnAnalyzeEyeMovementsDuringMicroStimAsFunctionOfDepthAux(strctKofiko,strctSync,strRawFolder,strSession, iChannelID, strctAdvancersInformation,strctStatServer,afTrainsGrass1TS, afAmplitude1,strOutputFolder,'Grass1',afUniqueDepthMM(iDepthIter),abFixationGrass1);
    end
    if length(afTrainsGrass2TS) > 0
        fnAnalyzeEyeMovementsDuringMicroStimAsFunctionOfDepthAux(strctKofiko,strctSync,strRawFolder,strSession, iChannelID, strctAdvancersInformation,strctStatServer,afTrainsGrass2TS, afAmplitude2,strOutputFolder,'Grass2',afUniqueDepthMM(iDepthIter),abFixationGrass2);
    end
    if length(afTrainsBothGrass)
        fnAnalyzeEyeMovementsDuringMicroStimAsFunctionOfDepthAux(strctKofiko,strctSync,strRawFolder,strSession, iChannelID, strctAdvancersInformation,strctStatServer,afTrainsBothGrass, afAmplitudeBoth, strOutputFolder,'Grass1and2',afUniqueDepthMM(iDepthIter),abFixationGrassBoth);
    end
end
...
function fnAnalyzeEyeMovementsDuringMicroStimAsFunctionOfDepthAux(strctKofiko,strctSync,strRawFolder,strSession, iChannelID, strctAdvancersInformation,strctStatServer,afTrainOnset_PLX, afAmplitude, strOutputFolder, strTriggerChannel, fDepthMM, abFixationON)
...
for iAmpIter=1:length(afUniqueAmplitudes)
    ...
    strctStat.m_abFixationSpotOnScreen = abFixationON(abRelevantTrains);
...
if ~isempty(aiStimulationDuringPassiveFixation)
    % Tricky bit..
      ...
      % First, the easy part: check the size of fixation spot...
      ...
      abFixationOnScreen(aiStimulationDuringPassiveFixation(afFixationSpotSizePix>0)) = true;
       % hard case.
       
       aiStimulationTrainsWithZeroFixationSpot = aiStimulationDuringPassiveFixation(afFixationSpotSizePix == 0);
       % We assume there is no OFF period.
       % we further assume that there are two main stimulus type (with
       % fixation spot and without fixation spot).
       % 
       afStimulation_TS_Kofiko = fnTimeZoneChange(afAllTrainOnsetsTS_Sorted(aiStimulationTrainsWithZeroFixationSpot),strctSync,'Plexon','Kofiko');
      [acImageNameOnScreenDuringStim, aiDesignIndex, aiStimInDesignIndex] = fnIdentifyWhichImageWasOnScreenDuringPassiveFixation(strctParadigm, strctSync, afStimulation_TS_Kofiko);

      % Now decide based on the image name whether a fixation spot was on
      % or not...
      abFixationOnScreen(aiStimulationTrainsWithZeroFixationSpot) = ismember(acImageNameOnScreenDuringStim,{'FixateGray' ,   'FixateGrayStim1'  ,  'FixateGrayStim12'   , 'FixateGrayStim2' });
end
return;



function [acImageNameOnScreenDuringStim, aiDesignIndex, aiStimInDesignIndex] = fnIdentifyWhichImageWasOnScreenDuringPassiveFixation(strctParadigm, strctSync, afSampleTS_Kofiko)
...
aiStimulusTS_Kofiko = strctParadigm.Trials.Buffer(5,:);
aiStimuliOnScreenDuringTrain = fnMyInterp1(aiStimulusTS_Kofiko, aiStimulusIndex,afSampleTS_Kofiko);
aiStimInDesignIndex = aiStimuliOnScreenDuringTrain;

% But we still don't know which design it was!
[acDesignNames, Dummy, aiMappingToUniqueList] = unique(strctParadigm.ImageList.Buffer);
aiActiveDesign = fnMyInterp1(strctParadigm.ImageList.TimeStamp,aiMappingToUniqueList, afSampleTS_Kofiko);
aiDesignIndex =aiActiveDesign;
acImageNameOnScreenDuringStim = cell(1, length(afSampleTS_Kofiko));
for iStimIter=1:length(afSampleTS_Kofiko)
    iActiveDesignForThisStim = aiMappingToUniqueList(aiActiveDesign(iStimIter));
    if ~isfield(strctParadigm,'Designs') &&  strcmpi(acDesignNames{iActiveDesignForThisStim},'\\192.168.50.93\StimulusSet\PassiveFixationMicrostim\Fixate_Microstim2Ch_NoFixation.xml')
          acMediaName = {'FixateGray'    'FixateGrayStim1'    'FixateGrayStim12'    'FixateGrayStim2'    'GrayStim1'    'GrayStim12'    'GrayStim2'};
          acImageNameOnScreenDuringStim{iStimIter} =acMediaName{aiStimuliOnScreenDuringTrain(iStimIter)};
    else
        if aiStimuliOnScreenDuringTrain(iStimIter) <= length(strctParadigm.Designs.Buffer{iActiveDesignForThisStim}.m_acMediaName)
            acImageNameOnScreenDuringStim{iStimIter} = strctParadigm.Designs.Buffer{iActiveDesignForThisStim}.m_acMediaName{aiStimuliOnScreenDuringTrain(iStimIter)};
        else
            acImageNameOnScreenDuringStim{iStimIter} = 'BUG?';
        end
    end
end
